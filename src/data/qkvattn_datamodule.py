...
from src.data.components.attn.datagen import QKVAttn_generate

# from components.attn.datagen import QKVAttn_generate # For single file debug


class QKVAttnDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            input_file_path = (
                self.hparams.data_dir
                + f"qkvattn_T{self.seq_length}_N{self.train_size + self.val_size + self.test_size}_D{self.input_dim}_inputs.npy"
            )
            output_file_path = (
                self.hparams.data_dir
                + f"qkvattn_T{self.seq_length}_N{self.train_size + self.val_size + self.test_size}_D{self.input_dim}_outputs.npy"
            )
