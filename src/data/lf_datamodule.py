...
# from components.lf.datagen import LF_generate
...
class LFDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            ...
            input_file_path = (
                self.hparams.data_dir
                + f"lf_{self.rho_name}_T{self.seq_length}_N{self.train_size + self.val_size + self.test_size}_dt{self.dt}_inputs.npy"
            )
            output_file_path = (
                self.hparams.data_dir
                + f"lf_{self.rho_name}_T{self.seq_length}_N{self.train_size + self.val_size + self.test_size}_dt{self.dt}_outputs.npy"
            )
