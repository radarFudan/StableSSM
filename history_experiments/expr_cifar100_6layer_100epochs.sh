...
for (( rep=1; rep<=num_repetitions; rep++ )); do
    ...
    for lr in "${learning_rates[@]}"; do
        ...
        for param in "${parameterizations[@]}"; do
            ...
            python src/train.py \
                experiment=CIFAR100/ssm \
                task_name="CIFAR100_StableSSM_Run${rep}_LR${lr}_${param}" \
                trainer.devices=1 \
                model.net.parameterization="$param" \
                logger=many_loggers \
                model.optimizer.lr="$lr" \
                seed=$rep \
                model.net.n_layers=6 \
                logger.wandb.project="StableSSM_CIFAR100_SSM_6Layers_100epochs" \
